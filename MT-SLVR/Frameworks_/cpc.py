...
###############################################################################
class CPC(nn.Module):
    def __init__(self, backbone, gar_hidden, batch_size):
    ...
    # def get_latent_representations(self, x, extra_params):
    #     """
    ...
    #     z = self.backbone(x, **extra_params).unsqueeze(1)
    ...
    def get_autoregressor_latent(self, z):
        # Given that we take backbone encoded inputs
        z = z.unsqueeze(1)
        z = z.permute(0, 2, 1)  # swap L and C
        c = self.autoregressor(z)
        return z, c
    ...
    #     z, c = self.get_latent_representations(x, extra_params=extra_params)
