...
from Frameworks_.aug_prediction import PredictAug
...
from Dataset_.dataset_utils import ContrastiveTransformations, batch_to_log_mel_spec, \
    batch_to_log_mel_spec_plus_stft, nothing_func
...
def setup_func(params, aug_params, device, restart=False, checkpoint_data=None):
    ...
    # FRAMEWORK SELECTION
    #########################
    framework = PredictAug(model, out_dim=len(possible_augs))
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimiser, T_max=params['training']['val_gap'], 
