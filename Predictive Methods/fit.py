...
from aug_functions import gen_multi_label_augs, apply_basic_aug_funcs
from utils import save_checkpoint, save_backbone, load_checkpoint, load_backbone, EarlyStopping
...
def fit_func(framework, prep_batch_fns, dataloaders, optimiser, lr_sched, params, possible_augs, device):
    ...
    epoch_loop.set_postfix({'Train Loss': 0, 'Val Loss': 0, 'Val mAP':0})

    # Creates a path to save best model to
    best_model_path = os.path.join(params['base']['results_path'], params['base']['experiment_name'] + '.pt')
    ...
    for epoch in epoch_loop:
        ...
        for idx in range(params['training']['val_gap']):
            ...
            x = apply_basic_aug_funcs(samples=x, 
                augs_to_apply=augs_to_apply,
                batch_wise=params['aug_control']['batch_wise'])

            x = additional_batch_work(x, params['ft_params'])

            loss, mean_ap = framework.forward(x, y)
        ...
        avg_train_ap = train_mean_ap / params['training']['val_gap']

        avg_val_loss = val_loss / len(valid_loader)
        avg_val_ap = val_mean_ap / len(valid_loader)
        ...
        # Write avg accuracies to tensorboard
        writer.add_scalar("mAP/avg_train", avg_train_ap, epoch)
        writer.add_scalar("mAP/avg_val", avg_val_ap, epoch)

        # Update visual loop bar
        loss_dict = {'Train Loss': round(avg_train_loss, 2),
             'Val Loss': round(avg_val_loss, 2), 
             'val mAP': round(avg_val_ap, 2)}
