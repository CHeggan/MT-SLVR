...
from Dataset_.dataset_utils import per_sample_scale, nothing_func, given_stats_scale, enforce_length
...
class Sample_Plus_Label_Dataset(Dataset):
    def __init__(self, data_path, norm, variable=False, eval=False, expected_files=None, 
        class_weights=None, stats=None, df_path=None, ext='.pt', label=True):
        ...
        Args:
            ...
            ext (str, optional): The extension that the files of interest have. Defaults
                to .pt
            label (bool, optional): Whether or not teh label is in fact in teh loaded file. 
                Defaults to True
        """  
        self.ext = ext      
        self.norm = norm
    ...
    def __getitem__(self, item):
        ...
        sample_path = os.path.normpath(os.path.join(self.data_path, str(self.expected_files[item]) + self.ext))
        ...
        if self.label:
            ...
            if self.ext == '.npy':
                ...
                # Loads in just the sample
                sample = torch.load(sample_path)

            # Create fake labels here to avoid refactoring some code
            # Justify as the entire code shouldnt be using labels anyway
            one_hot_labels = torch.zeros(size=(1,))
        ...
        if not self.eval:
            ...
            if sample.ndim > 2:
                ...
                sample = sample[idx]
        # If we are using variable length data (but have multiple sets i.e metaaudio)
        #   we need some additional catches
        if self.variable:
            if sample.ndim > 1:
                if sample.shape[0] == 1:
                    pass
