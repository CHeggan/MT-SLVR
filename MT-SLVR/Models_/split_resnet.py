"""
Split ResNet model modules and classes. Contains modified code that allows for 
...
class SplitResNet(nn.Module):
    ...
    def __init__(self, block, layers, dims, out_dim_list, in_channels=1, zero_init_residual=False,
...
def split_resnet18(dims, fc_out, in_channels):
    return SplitResNet(block=BasicBlock, layers=[2, 2, 2, 2], dims=dims, out_dim_list=fc_out, in_channels=in_channels)

def split_resnet34(dims, fc_out, in_channels):
    return SplitResNet(block=BasicBlock, layers=[3, 4, 6, 3], dims=dims, out_dim_list=fc_out, in_channels=in_channels)

def split_resnet50(dims, fc_out, in_channels):
    return SplitResNet(block=Bottleneck, layers=[3, 4, 6, 3], dims=dims, out_dim_list=fc_out, in_channels=in_channels)

def split_resnet101(dims, fc_out, in_channels):
    return SplitResNet(block=Bottleneck, layers=[3, 4, 23, 3], dims=dims, out_dim_list=fc_out, in_channels=in_channels)

def split_resnet152(dims, fc_out, in_channels):
    return SplitResNet(block=Bottleneck, layers=[3, 8, 36, 3], dims=dims, out_dim_list=fc_out, in_channels=in_channels)
...
# EXAMPLE CALL
###############################################################################
# model = split_resnet18(dims=2, in_channels=1, fc_out=[5000, 30, 20]).to('cuda')

# data = torch.rand(10, 1, 128, 313).to('cuda')

# print(model.forward(data, 'all').shape)
